...
def calculate_frequency_TE(df, country_name):
    ...
    # remove column count_0/0
    df = df.drop(columns=['count_0/0'])
...
# get sizes of TEs
df['TE_size'] = df['position_end'] - df['position_start']
# plot histogram of TE sizes
plt.figure(figsize=(10, 6))
plt.hist(df['TE_size'], bins=50, color='skyblue', edgecolor='black')
plt.title('Distribution of TE Sizes')
plt.xlabel('TE Size (bp)')
plt.ylabel('Frequency')
plt.yscale('log')  # Use logarithmic scale for better visibility
plt.grid(axis='y', alpha=0.75)
plt.savefig('TE_size_distribution.png')
plt.close()

# calculate intervals to split TEs by size - small, medium, large - calculate quantiles 33 and 66
size_33 = df['TE_size'].quantile(0.33)
size_66 = df['TE_size'].quantile(0.66)
print(f'Size 33%: {size_33}, Size 66%: {size_66}')
...
for size in sizes:
        ...
        print(size)
        # make df for each size - small < size_33, medium between size_33 and size_66, large > size_66
        if size == 'small':
            te_df = df[df['TE_size'] <= size_33].copy()
            better_name = 'small_TEs'
        elif size == 'medium':
            te_df = df[(df['TE_size'] > size_33) & (df['TE_size'] <= size_66)].copy()
            better_name = 'medium_TEs'
        elif size == 'large':
            te_df = df[df['TE_size'] > size_66].copy()
            better_name = 'large_TEs'
        elif size == 'very_very_small':
            te_df = df[df['TE_size'] < 3].copy()
            better_name = 'very_very_small'
        elif size == 'all_sizes':
            te_df = df.copy()
            better_name = 'all_sizes'

        # Split data by country
        for i, country in enumerate(countries):

            # Get all columns for this country from the renamed genome columns
            country_cols = [col for col in te_df.columns 
            ...
            country_df = te_df[base_cols + country_cols]
            ...
            print(f'total number of TEs in {country}: {len(frequency_df)}')
            ...
            print(f'number of non matched rows: {len(non_matched_rows)}')
            ...
            print(country)
            print(percentage_overlapping[country])
        ...
        percentages = [percentage_overlapping[country]['percentage'] * 100 for country in countries]
        print(percentages)
        # Create the bar plot
        ...
        # Show the plot
        plt.savefig(f'percentage_TEs_overlapping_genes_new_{better_name}.svg')
        ...
        for country, dfs in all_dfs_ref.items():
            matched_df, not_overlapping = dfs

            if matched_df.empty and not_overlapping.empty:
                print(f"No data for {country}, skipping...")
                continue

            gene_freq_overlapping = np.nan
            gene_freq_not_overlapping = np.nan

            if not matched_df.empty:
                attribute_columns = matched_df['attributes'].str.split(';', expand=True)
                gene_col = attribute_columns[2]  # column with gene ID

                gene_counts = gene_col.value_counts()
                ...
                for gene_id in top_gene_ids:
                    freqs.append(matched_df[gene_col == gene_id]['frequency'].mean())
                gene_frequencies[country] = np.mean(freqs)

                gene_freq_overlapping = matched_df['frequency'].mean()

            if not not_overlapping.empty:
        ...
        # Plot
        df_plot.plot(kind='bar', figsize=(10,7), color=['darkblue','lightblue'], edgecolor='black')
        plt.xlabel("Country", fontsize=15)
        plt.ylabel("Mean TE Frequency", fontsize=15)
        plt.title(f"Mean TE Frequency: Overlapping vs. Not Overlapping Exons - {better_name}")
        plt.xticks(rotation=45, fontsize=12)
        plt.yticks(fontsize=12)
        plt.legend(title="TE Type")
        plt.tight_layout()
        plt.savefig(f'TE_frequency_overlapping_vs_not_new_{better_name}.svg')
        plt.close()

'''# === Plot 2: All Top Genes (multiple genes per country) ===
        plot_data = []
        for country, gene_list in top_genes.items():
            for gene_id,a count in gene_list:
        ...
        plt.tight_layout()
        #plt.savefig(f'all_top_genes_per_country_new_{better_name}.svg')
        ...
        plt.xticks(rotation=45)
        fig.tight_layout()
        #plt.savefig(f'combined_top_gene_freq_and_counts_new_{better_name}.svg')'''
