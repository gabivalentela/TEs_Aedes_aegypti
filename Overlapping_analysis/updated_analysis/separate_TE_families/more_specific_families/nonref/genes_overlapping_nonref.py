...
# get unique TEs
unique_TEs = df['TE_name'].unique()
#unique_TEs = unique_TEs[:15]
TE_names_list = set()
for TE_name in unique_TEs:
    #print("Original:", TE_name)

    match = re.search(r"(rnd_\d+_family_\d+_.+?)(?:_span.*)?$", TE_name)
    
    if match:
        better_name = match.group(1)
        #better_name = better_name.split("_")[0]
        TE_names_list.add(better_name)

print('TE_names_list:', TE_names_list)

for better_name in TE_names_list:
        all_dfs_ref = {}
        percentage_overlapping = {}
        country_vcf_dict_ref = {}
        country_pairs_nonref = list(combinations(countries, 2))
        print(better_name)
        # make df with only TE name - if better name is in the TE_name column
        te_df = df[df['TE_name'].str.contains(better_name, na=False)]
        ...
        for i, country in enumerate(countries):
            ...
            percentage_overlapping[country] = {
            'number_te_overlapping': TE_overlapping, 
            'total_number_TEs': total_number_TEs, 
            'percentage': TE_overlapping/total_number_TEs if total_number_TEs > 0 else 0}# Save the merged DataFrame for this country
