...
def plot_SFS(all_dfs, te_name, name="SFS"):
    """
    ...
    for country, df in all_dfs.items():
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"{name}_{country}_{te_name}_all_categories.svg")
        plt.close()
        ...
        for category in categories:
            ...
            plt.xticks(x, [f"{val:.2f}" for val in plot_df.index], rotation=45)
            plt.tight_layout()
            plt.savefig(f"{name}_{country}_{category}_{te_name}.svg")
            plt.close()'''
...
unique_TEs = unique_TEs[:2]
TE_names_list = []
# fix the TE names
for TE_name in unique_TEs:
    #print("Original:", TE_name)

    match = re.search(r"(rnd_\d+_family_.+?)(?:_span.*)?$", TE_name)
    if match:
        better_name = match.group(1)
        TE_names_list.append(better_name)
        ...
        for i, country in enumerate(countries):
            ...
            percentage_overlapping_rep[country] = {
                'number_te_overlapping': TE_overlapping_rep,
                'total_number_TEs': total_number_TEs_rep,
                'percentage': TE_overlapping_rep / total_number_TEs_rep if total_number_TEs_rep > 0 else 0
            }
        ...
        for country, data in all_categories.items():
            # count how many categories are non-empty
            non_empty = sum(len(df) > 0 for df in data.values())

            # require at least 2 categories to be non-empty
            if non_empty < 2:
                #print(f"  Skipping (fewer than 2 categories with data) - {better_name}")
                continue

            # otherwise, safe to process/plot
            for category, df in data.items():
                print(f"  {category}: {len(df)}")

            categories_examples = SFS_data(all_categories)
            plot_SFS(categories_examples, better_name)
