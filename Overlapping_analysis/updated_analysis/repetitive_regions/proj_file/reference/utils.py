...
def read_data_annotation(path_annotation):
    ...
    file_ext = os.path.splitext(path_annotation)[1].lower()

    if file_ext == ".gff" or file_ext == ".gff3":
        data = pd.read_csv(
            path_annotation,
            sep='\t',
        ...
        # BED files usually have at least 3 columns (chrom, start, end)
        # Some versions include additional columns
        bed_columns = [
            'chrom', 'start', 'end'
        ]
        ...
        raise ValueError("Unsupported file format. Please provide a GFF or BED file.")
