    ...
    for sfs in list_of_neutral:
        ...
        if match:
            ...
            if country_name_to_match == country_name:
                ...
                neutral_new_df = neutral_new_df.rename(columns={"variable": "occurence", "value": "SNPs"})
                total_neutral = neutral_new_df['SNPs'].sum()
                proportions_neutral = neutral_new_df['SNPs'] / total_neutral
...
def plot_sfs_master_df(ax, df, suf, country):
    ...
    ax.set_title(f"{country} - {suf}", fontsize=45)
...
reference_path = '/work/users/g/a/gabivla/lab/SV_mosquitos/VCF_genotypes_TEs/separate_chromosomes/merged_reference/VCF_reference_TEs_post_processing.csv'
...
df_nonref = pd.read_csv(nonreference_path)
...
genome_cols_nonref = [col for col in df_nonref.columns if col not in base_cols]

# Create rename dictionary and apply it once
rename_dict_ref = {col: f"{col}_{location_dict.get(col, '')}" for col in genome_cols_ref}
rename_dict_nonref = {col: f"{col}_{location_dict.get(col, '')}" for col in genome_cols_nonref}
...
# Extract unique countries from the renamed genome columns only - nonreference
countries_nonreference = set()
for original_col in genome_cols_nonref:
    new_col = rename_dict_nonref[original_col]
    country = new_col.split('_')[-1]
    countries_nonreference.add(country)
...
# Non reference TEs
df_nonreference = pl.DataFrame(df_nonref)
TE_types_nonref = count_TE_occurence(df_nonreference)
TE_types_nonref = TE_types_nonref['TE_name'].to_list()
df_nonref = df_nonreference.to_pandas()
...
for i, country in enumerate(countries_reference):
    ...
    for j, top_te in enumerate(TE_types_ref):
        country_df = pl.DataFrame(country_df)
        te_content_all_genomes_ref = split_by_te_type_nonref(top_te, country_df)
        ...
        df_countries[f'{top_te}_{country}_ref'] = frequency_TEs

#---------------------------------------------------------------------------------------------
# loop for non reference

# Split data by country
for i, country in enumerate(countries_nonreference):

    # Get all columns for this country from the renamed genome columns
    country_cols = [col for col in df_nonref.columns 
                   if col not in base_cols and col.endswith('_' + country)]
    ...
    # Create a subset with base columns and columns for this country
    country_df = df_nonref[base_cols + country_cols]
    ...
    for j, top_te in enumerate(TE_types_nonref):
        country_df = pl.DataFrame(country_df)
        te_content_all_genomes_nonref = split_by_te_type_nonref(top_te, country_df)
        te_content_all_genomes_nonref = te_content_all_genomes_nonref.to_pandas()
        
        # calculate frequency for each TE
        frequency_TEs = calculate_frequency_TE(te_content_all_genomes_nonref, country)
        df_countries[f'{top_te}_{country}_nonref'] = frequency_TEs
...
for key, value in df_countries.items():
    # Extract `top_te`, `country`, and `ref_or_nonref` using regex
    match = re.search(r'([A-Z_]+)_([A-Za-z]+)_(.+)', key)
    if match:
        top_te = match.group(1)
        country = match.group(2)
        ref_or_nonref = match.group(3)

        # Initialize country dictionary if it doesn't exist
        if country not in country_dfs:
            country_dfs[country] = {}

        # Combine `ref` and `nonref` for the same `top_te` within a country
        if top_te in country_dfs[country]:
            country_dfs[country][top_te] = pd.concat([country_dfs[country][top_te], value], ignore_index=True)
        else:
            # Initialize with the current DataFrame
            country_dfs[country][top_te] = value
...
for country, te_dfs in country_dfs.items():
    row = i // 4
    col = i % 4
    ...
    for top_te, df in te_dfs.items():
        sfs_data_counts = sfs_data(df, top_te)
        sfs_data_dict[top_te] = sfs_data_counts
        list_of_dataframes.append(sfs_data_counts)

    master_df = sfs_general_data(list_of_dataframes)
    master_df.to_csv(f'{country}_master_df.csv')
    all_info_df = add_other_information(master_df, neutral_data_path, country)
    folded_dataframe = fold_my_sfs(all_info_df)

    # Add the plot for the current country
    plot_sfs_master_df(axs[row, col], folded_dataframe, 'all', country)

    # Set the title for the subplot
    axs[row, col].set_title(f"{country}", fontsize=30)

    i += 1  # Increment subplot index

plt.tight_layout()
plt.savefig(f'new_combined_SiteFrequencySpectrum.svg')