...
def main():
    ...
    for i, (country_ref, df_country_ref) in enumerate(ref_country.items()):
        ...
        te_content_all_genomes_ref_SNPcheck = Making_vcf.remove_duplicates(te_content_all_genomes_ref_SNPcheck)
        # making VCF file
        ...
        vcf_file.write_csv(f'/work/users/g/a/gabivla/lab/SV_mosquitos/VCF_genotypes_TEs/separate_chromosomes_windows_reference/reference_initial_files//VCF_TEs_reference_{country_ref}_familiesremove_2.csv')
        # add vcf_files to dict
        vcf_files[country_ref] = vcf_file

    # merge all VCFs
    vcf_all_countries = Making_vcf.merge_vcf_files(vcf_files)
    vcf_all_countries = Making_vcf.remove_duplicates_final(vcf_all_countries)
    ...
    output_dir = f'/work/users/g/a/gabivla/lab/SV_mosquitos/VCF_genotypes_TEs/separate_chromosomes_windows_reference/reference_initial_files/{chromosome}'
    ...
    for (chromosome, window_id), group in vcf_all_countries.group_by(["chromosome", "window_id"]):
        ...
        output_file = os.path.join(output_dir, f"VCF_TEs_reference_{chromosome}_{window_id}_initial.csv")
