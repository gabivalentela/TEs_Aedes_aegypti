...
class Making_vcf:
    ...
    def update_vcf(vcf_file, updated_genotypes):
        ...
        for te_info, genotype in updated_genotypes.items():
            ...
            match = re.search(r'(.+)_([A-Z].+)_(\d+)_(\d+)_(.+)', te_info)
            if not match:
    ...
    def split_vcf_into_windows(df, window_size):
        """
        Split the VCF DataFrame into windows of a specified size, ensuring overlapping variants 
        are included in multiple windows if necessary.
        ...
        for chrom in chromosomes:
            ...
            for window_id, start_pos in enumerate(range(0, max_pos, window_size)):
                ...
                window_df = chrom_df.filter(
                    (pl.col('position_start') < end_pos) & (pl.col('position_end') > start_pos)
                ).with_columns(pl.lit(window_id).alias("window_id"))  # Add window ID

                window_dfs.append(window_df)

        return pl.concat(window_dfs) if window_dfs else pl.DataFrame()
