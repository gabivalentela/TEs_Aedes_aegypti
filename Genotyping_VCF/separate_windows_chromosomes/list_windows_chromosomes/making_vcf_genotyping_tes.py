...
def main():
    ...
    # split chromosomes into windows
    vcf_all_countries = Making_vcf.split_vcf_into_windows(vcf_all_countries, 100000)
    # making list of windows for each chromosomes
    window_values = vcf_all_countries['window_id'].unique().to_list()
    outfile = open(f'list_windows_chr_{chromosome}.txt', 'a')
    for window in window_values:
        outfile.write(f'{chromosome} {window}\n')
    outfile.close()
