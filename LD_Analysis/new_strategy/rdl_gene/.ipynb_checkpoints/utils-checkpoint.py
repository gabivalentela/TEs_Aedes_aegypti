...
class LD_analysis:
        ...
        for variant in variant_info:
            ...
            for record in vcf_file:
                if record.pos == position_start:
                    # Initialize a dictionary to hold genotype information for this variant
                    genotype_info = {'Variant': name}
                    for sample in record.samples:
                        genotype = record.samples[sample]['GT']
                        genotype_info[sample] = "/".join(map(str, genotype)) if genotype is not None else "./."

                    # Append the information as a new row in the list
                    variant_info_list.append(genotype_info)

                    # Break the loop as we found the position we are interested in
                    break
    ...
    def add_country_name_SNPs(df, file_loc): 
        ...
        # Create the 'space' column in the info_location_country DataFrame
        #info_location_country['space'] = info_location_country['country'] + '_' + info_location_country['location']
        info_location_country['space'] = info_location_country['country']
        
        # Get the list of column names from df
        match_list = df.columns

        # Create a dictionary to map Genome_code to space
        genome_space_dict = pd.Series(info_location_country.space.values, index=info_location_country.Genome_code).to_dict()

        # Update column names in df with space_info
        new_columns = []
    ...
    def remove_fixed_TEs_country(df):
        # Extract the country from the genome code
        df['Country'] = df['Genome_code'].str.split('_').str[-1]
        
        # Count the number of unique genome codes for each country
        unique_counts = df.groupby('Country')['Genome_code'].nunique()
        
        # Print the number of genomes each country has
        print("Number of genomes for each country:")
        print(unique_counts)
        
        # Add the number of genomes to the dataframe
        ...
        # Count the frequency of occurrences for each TE in the grouped dataframe
        ...
        # Create a set of tuples with the columns of interest from fixed_TEs
    ...
    def make_list_of_te_insertions(gene_df):
        ...
        # Edit TE_name to get only the first item before the |
        selected_columns_df.loc[:, 'TE_name'] = selected_columns_df['TE_name'].str.split('|').str[0]
        
        # Group by specified columns and join Genome_code with ;
    ...
    def make_table_insertion(unique_df):
        # Create a new column for the unique identifier combining chromosome, position_start, position_end, and TE_name
        ...
        # Split Genome_code into a list of codes
        unique_df['Genome_code'] = unique_df['Genome_code'].str.split(';')

        # Create a list of all unique genome codes
        all_genome_codes = sorted(set(code for codes in unique_df['Genome_code'] for code in codes))

        # Create a dictionary to store the presence of each genome code for each TE insertion
        presence_dict = {}
    ...
    def merge_with_te_table(df_snp, df_te):
        ...
        # Concatenate the dataframes along rows, ignoring original indices
    ...
    def merge_presence_with_variants(presence_table_nonref, all_variants_df):
        # Reset index for all_variants_df and add '_Colombia' to its columns
        ...
        # Merge both DataFrames on matching columns
        merged_df = pd.concat(
    ...
    def remove_columns_with_none_pairs(df):
        ...
        # Iterate over each column and check if (None, None) exists in any row
        for col in df.columns:
            if any(df[col] == 'None/None'):  # Check if (None, None) is present in the column
                columns_to_remove.append(col)

        # Drop the columns that contain (None, None) pair
    ...
    def get_encoding_SNPs(df):
        variants = [
            'AaegL5_2_41847790_A301S']
        
        # Store a copy of all variant rows to encode
        variant_rows = df[df['Variant_name'].isin(variants)].copy()

        # Get the other rows (that should remain untouched)
        other_rows = df[~df['Variant_name'].isin(variants)].copy()
        # Duplicate rows for encoding_1 and encoding_2
        ...
        # Combine encoding rows and the other non-variant rows
    ...
    def calculate_LD_with_estimator(presence_table, significance_threshold):
        ...
        for i in range(len(te_identifiers)):
            for j in range(i + 1, len(te_identifiers)):
                ...
                if r_square is not None and r_square > significance_threshold and ('A301S' in te_id_1 or 'A301S' in te_id_2):
    ...
    def make_interpret_table(LD_values):
        # Extract unique TE identifiers from the dictionary keys
        te_identifiers = set()
        for te_pair in LD_values.keys():
            te_identifiers.update(te_pair)
        
        te_identifiers = sorted(te_identifiers)  # Sort identifiers for consistent ordering
        
        # Create an empty DataFrame with TE identifiers as both row and column indices
        ld_table = pd.DataFrame(index=te_identifiers, columns=te_identifiers)
        
        # Populate the DataFrame with the r values from the LD calculations
        for te_pair, values in LD_values.items():
            te1, te2 = te_pair
            ld_table.loc[te1, te2] = values['r']
            ld_table.loc[te2, te1] = values['r']  # Ensure symmetry
    ...
    def retrieve_significant_rsquared(df, significant_threshold, output_filename):
        ...
        col1 = 'AaegL5_2_41847790_A301S_encoding_1'
        col2 = 'AaegL5_2_41847790_A301S_encoding_2'
        ...
        # Filter: keep rows where either encoding_1 OR encoding_2 > threshold
        ...
        # Get the TEs from the filtered rows
    ...
    @staticmethod 
    def plot_ld_heatmap_r2(ld_table, list_TEs, output_file, highlight_variant='AaegL5_2_41847790_A301S'):
        import pandas as pd
        import numpy as np
        import matplotlib.pyplot as plt
        ...
        """
        # Convert the table to numeric, replacing non-numeric values with NaN
        numeric_table = ld_table.apply(pd.to_numeric, errors='coerce')
        
        # Create a mask for the upper triangle (values above the diagonal)
        mask = np.triu(np.ones_like(numeric_table, dtype=bool), k=1)
        
        # Apply the mask to the numeric table: Keep only the lower triangle (below diagonal)
        lower_triangle_table = numeric_table.where(mask == False)
        
        # Create the plot
        plt.figure(figsize=(20, 10), dpi=300)  # Increased figure size for better visibility
        
        # Create the heatmap with the original row and column order
        ...
        # Filter and prepare x-axis labels
        ...
        # Filter and prepare y-axis labels
        ...
        # Highlight the specified variant if it exists in the labels
    ...
    def sign_rsquared_retrieve_genotype(genotype_dict, significant_rsquared):
        ...
        for key, value in genotype_dict.items():
            for item in key:
                if item=='AaegL5_2_41847790_A301S':
    ...
    def same_genotype(genotype_df):
        if genotype_df.empty:
            return genotype_df, []  # Return empty DataFrame and empty list if input is empty

        # Convert the genotypes into strings for easier comparison
        genotype_df['Genotypes_str'] = genotype_df['Genotypes'].astype(str)

        # Get the genotype of the first row (assuming it's for 'V410L')
        first_row_genotype = genotype_df.iloc[0]['Genotypes_str']

        # Get the matching rows where the Genotypes are the same as the first row
        matching_rows = genotype_df[genotype_df['Genotypes_str'] == first_row_genotype]

        # If no matches are found, return an empty DataFrame and an empty list
        if matching_rows.empty:
            return matching_rows, []

        # Return the identifiers (first column) as a list and the matching rows
    ...
    def combining_TEs(complete_df, variation):
        ...
        # Use a regex to split only after the chromosome name and extract Chr, position_start, position_end, and TE_name
        ...
        # Convert position_start and position_end to integers
        ...
        # Initialize an empty list to store final rows
        final_rows = []
        num_removed_lines = 0  # Counter for removed lines
        number_of_chosen_rows = 0
        combined_groups = []  # Store combined group information
        ...
        for name, group in complete_df.groupby(group_cols):
            ...
            # Track chosen rows for the group
                    ...
                    if (abs(position_start_j - position_start_i) <= variation or
                        ...
                        for col in genotype_cols:
                            ...
                            if combined_value == (0, 0) and new_value != (0, 0):
                                ...
                                combined_row[col] = max(combined_value, new_value)  # Custom rule: choose the larger value
                        break
