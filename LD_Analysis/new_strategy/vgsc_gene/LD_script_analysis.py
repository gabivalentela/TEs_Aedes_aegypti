...
genes_with_regulatory_region = {
    'Chr': 'AaegL5_3',
    'position_start': 315926360 - 10000,  # initial position - 10 KB
    'position_end': 316405639 + 10000,    # final position + 10 KB
    'gene_name': 'VGSC'
}

#get SNPs info from VCF
variant_info = [{'variant_name': 'AaegL5_3_316080722_V410L', 'Chromosome': 'AaegL5_3', 'Position_start': 316080722}]
    
# variant separate dataset
variant_info_other = [{'variant_name': 'AaegL5_3_315983763_V1016I', 'Chromosome': 'AaegL5_3', 'Position_start': 315983763}, {'variant_name': 'AaegL5_3_315999297_I915K', 'Chromosome': 'AaegL5_3', 'Position_start': 315999297}, {'variant_name': 'AaegL5_3_316014588_S723T', 'Chromosome': 'AaegL5_3', 'Position_start': 316014588}]
df_variant_info_other = pd.DataFrame(variant_info_other)

vcf_path = '/nas/longleaf/home/gabivla/lab/SV_mosquitos/LD_analysis/SNP_data/filtered_variants.recode.vcf'
df_SNPs_variants = LD_analysis.find_variant_information(vcf_path, variant_info)
all_variants_df = LD_analysis.find_variant_information(vcf_path, variant_info_other)
...
complete_df_ref = LD_analysis.merge_presence_with_variants(complete_df_ref, all_variants_df)
complete_df_ref = LD_analysis.remove_columns_with_none_pairs(complete_df_ref)

# add both encoding of SNPs
complete_df_ref = LD_analysis.get_encoding_SNPs(complete_df_ref)

# Calculating LD
D_r_values_ref, genotype_data_ref = LD_analysis.calculate_LD_with_estimator(complete_df_ref, significance_threshold=0.5)
...
significant_rsquared_ref, list_significant_tes_ref = LD_analysis.retrieve_significant_rsquared(
    ld_table_ref, significant_threshold=0.5, output_filename='significant_r_squared_ref.csv'
)
...
complete_df_nonref = LD_analysis.merge_presence_with_variants(complete_df_nonref, all_variants_df)
...
# Calculating LD
D_r_values_nonref, genotype_data_nonref = LD_analysis.calculate_LD_with_estimator(complete_df_nonref, significance_threshold=0.5)
...
significant_rsquared_nonref, list_significant_tes_nonref = LD_analysis.retrieve_significant_rsquared(
    ld_table_nonref, significant_threshold=0.5, output_filename='significant_r_squared_nonref.csv'
)
LD_analysis.plot_ld_heatmap_r2(ld_table_nonref, list_significant_tes_nonref, output_file='ld_heatmap_r2_nonref.png')
