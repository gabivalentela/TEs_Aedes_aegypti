...
def plot_TE_counts(df):
    ...
    # Define a color mapping for each country (pastel greens)
    color_mapping = {
...
def plot_PCA_data(df):
    ...
    genotype_df = df.iloc[:, 6:]  # Assuming first 5 columns are metadata
    ...
    # Labels and title - make fontsize bigger
    ...
    # make the legend be outside of the plot - upper left
    plt.legend(title="Country", loc='upper left', fontsize=16)
...
#df1 = select_more_than_3(df, 1)
#df3 = select_more_than_3(df, 3)
#df5 = select_more_than_3(df, 5)
#df7 = select_more_than_3(df, 7)
#print(f'''Number of TEs with at least 1 occurrences: {df.shape[0]}
#Number of TEs with at least 3 occurrences: {df3.shape[0]}
#Number of TEs with at least 5 occurrences: {df5.shape[0]}
#Number of TEs with at least 7 occurrences: {df7.shape[0]}''')
...
for i, country in enumerate(countries):
    ...
    count_te_per_chr(country, country_df_new, 'nonref')
