...
def plot_TE_counts(df):
    ...
    color_mapping = {
        "Colombia": "#FEEBE2",  # Light Pink
        "USA": "#FBB4B9",  # Soft Red-Pink
        "Brazil": "#F768A1",  # Medium Pink
        "Kenya": "#DD3497",  # Deep Pink
        "Senegal": "#AE017E",  # Dark Magenta
        "Gabon": "#7A0177"  # Deep Purple
    }
    ...
    # Save the plot to a file
    plot_filename = f'top_TEs_by_percentage_nonreference.png'
    plt.savefig(plot_filename, bbox_inches='tight')
    plt.close()
...
def plot_PCA_data(df):
    ...
    # Create a color map
    country_colors = plt.cm.get_cmap('RdPu', len(unique_countries))
    ...
    # Save the figure
    plt.savefig('PCA_nonreference.png')
    plt.close()
...
# Ensure "Significance" is a flat string for CSV compatibility
results_df = results_df.with_columns(pl.col("Significance").cast(pl.Utf8))
results_df = results_df.drop(["Significance"])

# Write results to a CSV file
results_df.write_csv(f'chi_square_results_reference.csv')