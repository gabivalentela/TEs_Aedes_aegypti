...
for col in all_top10_diffs_df.columns:
    if col.startswith("count_1/1_"):  # Identify relevant columns
        ...
        if country in all_dfs_ref:  # Ensure the country exists in all_dfs_ref
            ...
            fill_values = fill_values[~fill_values.index.duplicated()]
...
# sort the df by TE_information
all_top10_diffs_df = all_top10_diffs_df.sort_values(by="TE_information").reset_index(drop=True)
all_top10_diffs_df.to_csv('all_top10_diffs_all_countries_reference.csv', index=False)
...
# Show plot
plt.tight_layout()
plt.savefig('mean_frequencies_reference.svg')
...
plt.tight_layout()
plt.savefig('correlation_matrix_reference.svg')
