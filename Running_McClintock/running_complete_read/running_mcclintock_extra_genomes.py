...
def run_mcclintock(fastq_path_general, files_fastq, mcclintock_path, genome_path, consesus_te_path):
    ...
    for name, fastqs in fastq_dict.items():
        ...
        if fastq_1 and fastq_2:
            print(f'{name}')
            print(f'sbatch -p general -n 24 --mem=80G -t 7-00:00:00 --mail-type=BEGIN,END,FAIL --mail-user=gabivla@email.unc.edu --wrap="python {mcclintock_path} -p 24 -r {genome_path} -c {consesus_te_path} -1 {fastq_1}  -2 {fastq_2} -o /work/users/g/a/gabivla/lab/SV_mosquitos/running_mcclintock/extra_genomes/output_{name} -m trimgalore,map_reads,temp2,ngs_te_mapper2"')
            os.system(f'sbatch -p general -n 24 --mem=80G -t 7-00:00:00 --mail-type=BEGIN,END,FAIL --mail-user=gabivla@email.unc.edu --wrap="python {mcclintock_path} -p 24 -r {genome_path} -c {consesus_te_path} -1 {fastq_1}  -2 {fastq_2} -o /work/users/g/a/gabivla/lab/SV_mosquitos/running_mcclintock/extra_genomes/output_{name} -m trimgalore,map_reads,temp2,ngs_te_mapper2"')
