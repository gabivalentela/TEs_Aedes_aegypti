...
def retrieve_srr_file(srr_dir):
    ...
    fastq_dict = {}
    if '.gz' in srr_dir and 'SRR11006834' in srr_dir:
        if '_1' in srr_dir:
            fastq_1 = f'{srr_dir}'
...
def edit_fastq_files(fastq_dict):
    for type_fastq, fastq_path in fastq_dict.items():
        if '1' in type_fastq:
            name = re.search(r'.+/original_(\w+)\_\w\.fastq.gz', fastq_path).group(1)
            output_file = f'/work/users/g/a/gabivla/data_mosquito/data_running_mcclintock/SRR_run_genome_test/multiple_srr_genomes/fastq_v4/{name}/{name}_1.fastq.gz'
            try:
                with gzip.open(fastq_path, 'rt') as input_fastq, gzip.open(output_file, 'wt') as output_fastq:
            ...
            name = re.search(r'.+/original_(\w+)\_\w\.fastq.gz', fastq_path).group(1)
            output_file = f'/work/users/g/a/gabivla/data_mosquito/data_running_mcclintock/SRR_run_genome_test/multiple_srr_genomes/fastq_v4/{name}/{name}_2.fastq.gz'
...
def main():
    ...
    general_path = '/work/users/g/a/gabivla/data_mosquito/data_running_mcclintock/SRR_run_genome_test/multiple_srr_genomes/fastq_v4/'
    list_of_directories = os.listdir(general_path)
    paths = []
    for directory in list_of_directories:
        if '.txt' not in directory:
            path = f'{general_path}{directory}'
            paths.append(path)
