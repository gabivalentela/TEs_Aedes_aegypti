...
for i, country in enumerate(countries):
    ...
    df_ir_genes.to_csv(f'./TEs_in_IR_genes_{country}_50.csv', index=False)'''
...
#csv_files = [file for file in list_file if file.endswith('.csv') and not file.endswith('_75.csv') and not file.endswith('_50.csv')]
csv_files = [file for file in list_file if file.endswith('_50.csv')]
...
for file in csv_files:
    ...
    if df.empty:
        ...
        df = pd.read_csv(file)
        country_name = re.search('TEs_in_IR_genes_(.+)_50.csv', file).group(1)
        ...
        pastel_palette = sns.color_palette("pastel", 6)
